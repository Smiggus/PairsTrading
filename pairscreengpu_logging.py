...
class PairScreener:
    ...
    def find_pairs(self, confidence_level=0.99):
        ...
        with tqdm(total=total_combinations, desc="Finding pairs") as pbar:
            for stock1 in self.cleaned_data.columns:
                sector1 = self.sector_mapping.get(stock1)
                if sector1 and sector1 != 'Unknown':
                    sector_stocks = [stock2 for stock2, sec in self.sector_mapping.items() if sec == sector1 and stock2 != stock1]
                    for stock2 in sector_stocks:
                        ...
                        if len(aligned_returns1) > 0 and len(aligned_returns2) > 0:
                            ...
                            if p_value < (1 - confidence_level):
                                pairs.append((stock1, sector1, stock2, self.sector_mapping[stock2], coint_stat, p_value, correlation))
                        pbar.update(1)
        ...
        pairs_df = pd.DataFrame(pairs, columns=['Stock 1', 'Sector 1', 'Stock 2', 'Sector 2', 'Cointegration Stat', 'P-Value', 'Correlation'])
